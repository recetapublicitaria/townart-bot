...
app.post("/whatsapp-webhook", async (req, res) => {
  ...
  const body = (req.body.Body || "").trim(); // texto que escribió la persona
  ...
  // Helper para detectar "sí" / "si" con o sin acento
  const lowerNoAccents = lower.normalize("NFD").replace(/[\u0300-\u036f]/g, "");

  try {
    // ¿La persona quiere agendar?
    ...
    if (session.step > 0 || quiereReservar) {
      ...
      if (session.step === 6) {
        if (lowerNoAccents.startsWith("si")) {
          // Confirmado ✅ → crear cita en Calendar
          await crearCitaCalendar(session, from);
          await sendWhats(
            from,
            "Listo 💜 Tu cita quedó agendada en Town Art Spa & Pole.\n" +
              "Si necesitas cambiar algo después, solo escríbeme por aquí."
          );
          sessions[from] = { step: 0 }; // flujo terminado
        } else {
          // NO → solo corregimos fecha y hora, mantenemos nombre/tipo/servicio
          session.step = 4;
          await sendWhats(
            from,
